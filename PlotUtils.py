...
def plotSimData(mdlsdata, show=False, filename=None):
    ...
    G = np.array(mdlsdata.sim_info['G'])
    ...
    if mdlsdata.sim_info['Nd_mode'] == 'discrete':
        ...
        ax3.bar(d, G, width, color='violet', label='simulated intensity distribution@90°')
        ...
        ax3.fill(d, G, color='violet', edgecolor=None, alpha=0.5, label='simulated intensity distribution@90°')
    ax3.set_xlim(0.7, 1e4)
    ax3.set_xscale('log')
    ax3.legend()
...
def plotTrainResultFromDict(train:dict, figure_title:str=None, show:bool=False, filename:str=None):
    '''train dict generated by MdlsNN.Train.toDIct()'''
    fig = plt.figure(figsize=(12, 8), dpi=300, facecolor='white')
    if figure_title != None:
        fig.suptitle(figure_title)
    ax1 = plt.subplot2grid((2,3), (0,0))
    ax1.plot(train['result']['epoch'], train['result']['loss'])
    ax1.set_yscale('log')
    ...
    plotAxisCorrelationFunction(
        ax2, 
        [dlsdata['tau'] for dlsdata in train['mdlsdata']['data'].values()],
        [dlsdata['g1square'] for dlsdata in train['mdlsdata']['data'].values()],
        '.'
    )
    plotAxisCorrelationFunction(
        ax2, 
        [dlsdata['tau'] for dlsdata in train['mdlsdata']['data'].values()],
        list(train['result']['fit_g1square'].values()),
        'k-'
    )

    ax3 = plt.subplot2grid((2,3), (1,0), colspan=3)
    if 'sim' in train['mdlsdata']['mode']:  #模拟数据画出真实分布
        sim_d = train['mdlsdata']['sim_info']['d']
        sim_N = train['mdlsdata']['sim_info']['N']
        sim_G = train['mdlsdata']['sim_info']['G']
        if train['mdlsdata']['sim_info']['Nd_mode'] == 'continuous':
            ax3.fill(sim_d, sim_N, color='lightskyblue', edgecolor=None, alpha=0.5, label='simulated number distribution')
            ax3.fill(sim_d, sim_G, color='violet', edgecolor=None, alpha=0.5, label='simulated intensity distribution@90°')
        else:  # discrete
            width = 0.1 * np.array(sim_d)  # automatically determine the width of bar according to data
            ax3.bar(sim_d, sim_N, width, color='lightskyblue', label='simulated number distribution')
            ax3.bar(sim_d, sim_G, width, color='violet', label='simulated intensity distribution@90°')
    train_d = np.array(train['result']['d'])
    train_N = np.array(train['result']['N'])
    train_G = np.array(train['result']['G'])
    if 'sim' in train['mdlsdata']['mode']:
        # 将训练结果的分布与模拟数据的分布拉平，暴力方法，没有经过细致分析
        train_N = train_N * (np.max(sim_N)/np.max(train_N))
        train_G = train_G * (np.max(sim_G)/np.max(train_G))
    plotAxisDiameterDistribution(ax3, train_d, train_N, log_d=True, color='blue', label='number weighted')
    plotAxisDiameterDistribution(ax3, train_d, train_G, log_d=True, color='fuchsia', label='intensity weighted@90°')
    ax3.legend()
...
if __name__ == '__main__':
    import json
    with open('data\simdata_trimodal_3_result-MESLoss-dmax=1e4.json', 'r') as f:
        train = json.load(f)
    plotTrainResultFromDict(train, show=False, filename='test.png')
